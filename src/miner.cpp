    ...
    {   
        ...
        *phash = block.GetHashYespower();
        ...
        if ((nNonce & 0xfff) == 0 || miner->fKill > 0) {
            if (miner->fKill > 0)
                LogPrintf("FairCoin miner kill flag > 0\n");
            return false;
        }
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash()) {
            LogPrintf ("FairCoin Miner: generated block is stale\n");
            return false;
        }
    }
    ...
    if (!ProcessNewBlock (chainparams, ptr, true, nullptr)) {
        LogPrintf ("FairCoin Miner: ProcessNewBlock, block not accepted\n");
        result =  false;
    }
...
{
    LogPrintf("FairCoin Miner started\n");
    ...
    try {
        while (true) {
            ...
            {
                LogPrintf("FairCoin Miner runtime error: %s\n", e.what());
                LogPrintf("FairCoin Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                MilliSleep(4*1000);
                vpwallets[0]->GetScriptForMining (coinbaseScript);
                continue;
            }
            ...
            LogPrintf("Running FairCoin Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (ScanHash (miner, pblock, nNonce, &hash))
                {
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("FairCoin Miner: proof-of-work found  \n  hash: %s  \ntarget: %s\n",
    ...
    {
        LogPrintf("FairCoin Miner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
